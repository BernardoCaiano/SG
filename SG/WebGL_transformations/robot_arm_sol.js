...
function render() {
    ...
    var pAngle = document.getElementById("pAngle").value;
    var zNear = document.getElementById("zNear").value/10.0;
    var zFar = document.getElementById("zFar").value/10.0;
    console.log(pAngle, zNear, zFar)
    p = perspective(pAngle, ratio, zNear, zFar);
    //console.log(pAngle, ratio, zNear, zFar, p)
    //p = ortho(-3, 3, -3, 3, zNear, zFar);
    gl.uniformMatrix4fv(projLoc, gl.FALSE, flatten(p));
    //console.log(p)

    var type = gl.LINE_LOOP;
    if (!wireframe)
        type = gl.TRIANGLES; // (to draw a solid cube)
    var start = 0; 
    var num = 36; 
    ...
    // move o objeto (MANTENDO o eixo de rotação)
    mv = mult(mv, translate(-2.0, 0.0, 0.0));   //position the shoulder
    //Shoulder Joint
    mv = mult(mv, rotate(shoulder, [0, 0, 1])); //rotate shoulder
    //Position Upper Arm Cube
    // move o objeto (ALTERANDO o eixo de rotação)
    mv = mult(mv, translate(1.0, 0.0, 0.0));    //rotation around center on (-1,0,0)
    //Scale and Draw Upper Arm
    matStack.push(mv); //save transformation (2nd - shoulder position and rotation - without scale)
    // assim, FUTUROS objetos com rotações não ficarão distorcidos
    ...
    // //  *************************/
    //Undo Scale
    mv = matStack.pop(); //retrieve transformation (2nd - shoulder position and rotation (to be applied also on ELBOW))
    // //Position Elbow Joint
    mv = mult(mv, translate(1.0, 0.0, 0.0));    //position the elbow
    //Elbow Joint
    mv = mult(mv, rotate(elbow, [0, 0, 1]));    //rotate elbow
